...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index+1:
        ...
        response = {
            'message':'Blockchain seems to deffer from local blockchain.'
        }
...
def add_transaction():
    ...
    if success:
        ...
        print('Creating a transaction failed')
        return jsonify(response),500
...
def get_chain():
    ...
    dict_chain = [ dict(dict_block,**{'transactions': [tx.__dict__.copy() for tx in dict_block['transactions']]})  for dict_block in[block.__dict__.copy() for block in chain_snapshot]]
